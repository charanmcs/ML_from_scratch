...
X = iris.data[:, [0, 3, 2]]  # Independent variables: 'sepal length (cm)', 'petal width (cm)', 'petal length (cm)'
y = iris.data[:, [1]]        # Dependent variable: 'sepal width (cm)'
...
model.fit(X_train, y_train, max_epochs=200, lr=0.001, regularization=0, patience=5)
...
plt.savefig("LinearRegression\modelPlots\\trainRegression_3.png")
plt.clf()

# Train a linear regression model with L2 regularization
model_v2 = LinearRegression()
model_v2.fit(X_train, y_train, max_epochs=200, lr=0.001, regularization=0.1, patience=5)
...
plt.savefig("LinearRegression\modelPlots\\trainRegressionWithL2_3.png")

# Save the weights of the model with L2 regularization
model_v2.save("LinearRegression\modelWeights\Regression3.pkl")
